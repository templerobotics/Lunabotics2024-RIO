...
public class DiggingBelt extends SubsystemBase {
    ...
    private final LunaSparkMax m_belt1;
    private final LunaSparkMax m_belt2;
    private final RelativeEncoder e_belt1;
    private final SparkMaxPIDController p_belt1;
    private final RelativeEncoder e_belt2;
    private final SparkMaxPIDController p_belt2;
    ...
    public DiggingBelt(){
        ...
        e_belt1 = m_belt1.getEncoder();
        p_belt1 = m_belt1.getPIDController();
        e_belt2 = m_belt2.getEncoder();
        p_belt2 = m_belt2.getPIDController();
        ...
        if (ENABLE_TEST_DASHBOARDS){
            reportInitialPID();
            pidConstants.put("belt-kP", LEADSCREW_kP); //BELT_kP
            pidConstants.put("belt-kI", LEADSCREW_kI); //BELT_kI
            pidConstants.put("belt-kD", LEADSCREW_kD); //BELT_kD
            pidConstants.put("belt-kIZ", LEADSCREW_kIZ); //BELT_kIZ
            pidConstants.put("belt-kFF", LEADSCREW_kFF); //BELT_kFF
            pidConstants.put("belt-setpoint", 0.0);
        }
    }
    ...
    public void runBelt(boolean reverse){
        int tempSpeed1 = -beltSpeed;
        int tempSpeed2 = beltSpeed;
        if (reverse){
            tempSpeed1 = tempSpeed1 * -1;
            tempSpeed2 = tempSpeed1 * -1;
        }
        p_belt1.setReference(tempSpeed1, ControlType.kVelocity);
        p_belt2.setReference(tempSpeed2, ControlType.kVelocity);
        beltRunning = true;
        networkTable.getEntry("beltSetpoint").setDouble(beltSpeed);
        shuffleboardEntries.get("belt-run").setBoolean(true);
    }
        ...
        for (String pidConstant : pidConstants.keySet()){
            ...
            switch(split[1]){
                ...
                case "kD":
                    ...
                    break;
                case "kIZ":
                    p_belt1.setIZone(newVal);
                    p_belt2.setIZone(newVal);
                    break;
                case "kFF":
                    p_belt1.setFF(newVal);
                    p_belt2.setFF(newVal);
                    break;
                case "setpoint":
                    //Update setpoint and turn belt off if needed
                    if (newVal == 0){
                    ...
                    p_belt1.setReference(newVal, ControlType.kVelocity);
                    p_belt2.setReference(newVal, ControlType.kVelocity);
                    break;
                default:
                    break;
            }
