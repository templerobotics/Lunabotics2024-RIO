...
public class DiggingLinearActuator extends SubsystemBase {
    ...
    private void linearUp() {
        if (a_linear1.getPosition() >= (LINEAR_MAX_TRAVEL - LINEAR_DEADBAND) || linearStateRight == LinearActuatorStateRight.Lowered) {
            m_linear1.stopMotor();
        
            return;
        }
        if (a_linear2.getPosition() >= (LINEAR_MAX_TRAVEL - LINEAR_DEADBAND) || linearStateLeft == LinearActuatorStateLeft.Lowered) {
            m_linear2.stopMotor();
